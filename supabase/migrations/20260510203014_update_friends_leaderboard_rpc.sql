-- Update friend leaderboard to return partitioned scores
...
BEGIN
  ...
  user_points AS (
    SELECT 
      ...
      SUM(
        CASE 
          WHEN p.type = 'semi1' AND p.is_qualifier = true AND r.is_semi1_qualifier = true THEN 3
          ELSE 0
        END
      )::INTEGER as semi1_score,
      SUM(
        CASE 
          WHEN p.type = 'semi2' AND p.is_qualifier = true AND r.is_semi2_qualifier = true THEN 3
          ELSE 0
        END
